...
static void PolygonCcwClipNegativeFaceAndProject(vec3ptr cp, f32 *cp_depth, u32 *cp_deepest, u32 *cp_count,struct stackVec3 clip_stack[2], const struct plane *face_plane, const u32 fv_count, constvec3ptr fv)
{
    ...
    vec3 tmp, side_plane_direction, inter;
	u32 cur = 0;

    /* Clip polygon against voronoi face region */
	for (u32 j = 0; j < fv_count; ++j)
	{
		const u32 prev = cur;
		cur = 1 - cur;
		stackVec3Flush(clip_stack + cur);

		Vec3Sub(tmp, fv[(j+1) % fv_count], fv[j]);
		Vec3Cross(side_plane_direction, tmp, face_plane->normal);
		struct plane side_plane = PlaneConstruct(side_plane_direction, fv[j]);
    ...
    *cp_count = 0;
	...
	for (u32 i = 0; i < clip_stack[cur].next; ++i)
	{
		...
		Vec3Sub(tmp, cp[*cp_count], fv[0]);
		cp_depth[*cp_count] = -Vec3Dot(tmp, face_plane->normal);

		if (cp_depth[*cp_count] >= 0.0f)
		{
			Vec3TranslateScaled(cp[*cp_count], face_plane->normal, cp_depth[*cp_count]);
...
static u32 HullContactInternalFaceContact(struct arena *mem_tmp, struct c_Manifold *cm, f32 *max_depth, const vec3 cm_n, const struct dcel *ref_dcel, const vec3 n_ref, const u32 ref_face_index, constvec3ptr v_ref, const struct dcel *inc_dcel, constvec3ptr v_inc)
{
	vec3 tmp1, tmp2, n;
...
u32 c_HullContact(struct arena *mem_tmp, struct c_Manifold *manifold, struct sat_Cache *cache, const struct sat_Cache *cache_copy, const struct c_Shape *s[2], const ds_Transform t[2], const u32 ref)
{
	...
	if (0.99f*f_query[0].depth >= e_query.depth || 0.99f*f_query[1].depth >= e_query.depth)
	{
		...
		if (f_query[0].depth > f_query[1].depth)
		{
            ...
            if (ref == 0)
            {
			    colliding = HullContactInternalFaceContact(mem_tmp, manifold, &max_depth, f_query[0].normal, h1, f_query[0].normal, f_query[0].fi, v1_world, h2, v2_world);
            }
            else
            {
                Vec3Scale(cm_n, f_query[0].normal, -1.0f);
			    colliding = HullContactInternalFaceContact(mem_tmp, manifold, &max_depth, cm_n, h1, f_query[0].normal, f_query[0].fi, v1_world, h2, v2_world);
                Vec3TranslateScaled(manifold->v[0], manifold->n, manifold->depth[0]);
                Vec3TranslateScaled(manifold->v[1], manifold->n, manifold->depth[1]);
                Vec3TranslateScaled(manifold->v[2], manifold->n, manifold->depth[2]);
                Vec3TranslateScaled(manifold->v[3], manifold->n, manifold->depth[3]);
		...
		else
		{
            ...
            if (ref == 1)
            {
			    colliding = HullContactInternalFaceContact(mem_tmp, manifold, &max_depth, f_query[1].normal, h2, f_query[1].normal, f_query[1].fi, v2_world, h1, v1_world);
            }
            else
            {
                Vec3Scale(cm_n, f_query[1].normal, -1.0f);
			    colliding = HullContactInternalFaceContact(mem_tmp, manifold, &max_depth, cm_n, h2, f_query[1].normal, f_query[1].fi, v2_world, h1, v1_world);
                Vec3TranslateScaled(manifold->v[0], manifold->n, manifold->depth[0]);
                Vec3TranslateScaled(manifold->v[1], manifold->n, manifold->depth[1]);
                Vec3TranslateScaled(manifold->v[2], manifold->n, manifold->depth[2]);
                Vec3TranslateScaled(manifold->v[3], manifold->n, manifold->depth[3]);
...
static u32 TriCcwPlaneHullContact(struct arena *mem_tmp, struct c_Manifold *cm, f32 *max_depth, const vec3 cm_n, const struct plane *tri_plane, const vec3 tri[3], const struct dcel *hull)
{
	...
	for (u32 fi = 0; fi < hull->f_count; ++fi)
	{
        ...
        const f32 dot_np = Vec3Dot(plane.normal_direction, tri_plane->normal);
	...
	for (u32 i = 0; i < inc_face->count; ++i)
	{
		...
		stackVec3Push(clip_stack + 0, hull->v[vi]);
	}
...
static u32 TriCcwHullEECheck(struct sat_EdgeQuery *query, const struct plane *tri_plane, const struct segment tri_s[3], const f32 tri_s_dist_sq[3], const vec3 tri_center, const struct dcel *hull, const u32 e2_1)
{
	...
	TriCcwNormalDirection(n2_1, hull->v[hull->e[hull->f[f2_1].first + 0].origin],  hull->v[hull->e[hull->f[f2_1].first + 1].origin], hull->v[hull->e[hull->f[f2_1].first + 2].origin]);
	TriCcwNormalDirection(n2_2, hull->v[hull->e[hull->f[f2_2].first + 0].origin],  hull->v[hull->e[hull->f[f2_2].first + 1].origin], hull->v[hull->e[hull->f[f2_2].first + 2].origin]);
	/* we are working with minkowski difference A - B, so gauss map of B is (-B). n2_1, n2_2 cross product stays the same. */
	Vec3NegateSelf(n2_1);	
	Vec3NegateSelf(n2_2);
...
static u32 TriCcwHullContact(struct c_Manifold *manifold, struct c_TriHullCache *new_cache, f32 *max_depth, const struct c_TriHullCache *old_cache, const vec3 tri[3], const struct dcel *hull, const u32 ref)
{
    /*
    ...
    *max_depth = -F32_INFINITY;
    ...
    //    default: { ds_AssertString("Should not happen."); } break;
    //}
    ...
    /* tri-plane vs. hull */
    {
        ProfZoneNamed("tri-plane vs. hull");
        ...
        {
            Vec3Copy(f_query[0].normal, tri_plane.normal);
            f_query[0].fi = 0;
            f_query[0].depth = min0_dist;
        }
        else
        {
            Vec3Scale(f_query[0].normal, tri_plane.normal, -1.0f);
            f_query[0].fi = 1;
            f_query[0].depth = min1_dist;
        }
    }
    ...
    {
        ProfZoneNamed("hull-planes vs. triangle");
    	for (u32 fi = 0; fi < hull->f_count; ++fi)
    	{
	...
	if (0.99f*f_query[0].depth >= e_query.depth || 0.99f*f_query[1].depth >= e_query.depth)
	{
		...
		else
		{
            ...
            if (ref == 1)
            {
			    colliding = HullContactInternalFaceContact(tmp, manifold, max_depth, f_query[1].normal, hull, f_query[1].normal, f_query[1].fi, hull->v, &hull_tri, hull_tri.v);
            }
            else
            {
                Vec3Scale(cm_n, f_query[1].normal, -1.0f);
			    colliding = HullContactInternalFaceContact(tmp, manifold, max_depth, cm_n, hull, f_query[1].normal, f_query[1].fi, hull->v, &hull_tri, hull_tri.v);
                Vec3TranslateScaled(manifold->v[0], manifold->n, manifold->depth[0]);
                Vec3TranslateScaled(manifold->v[1], manifold->n, manifold->depth[1]);
                Vec3TranslateScaled(manifold->v[2], manifold->n, manifold->depth[2]);
                Vec3TranslateScaled(manifold->v[3], manifold->n, manifold->depth[3]);
	    ...
	    while (it.sc--)
	    {
            ...
            { 
                ...
                for (u32 index = tri_first; index <= tri_last; ++index)
                {
                    ...
                    const struct c_TriHullCache *old_cache = NULL;
                    ...
                    f32 max_depth = -F32_INFINITY;
                    const u32 contact = TriCcwHullContact(&c->manifold, &c->cache, &max_depth, old_cache, tri, &hull_bvh_local_space, reference_index);
                    ProfZoneEnd;

                    {
                    ProfZoneNamed("NON-CACHED");
                    f32 dd = -F32_INFINITY;
                    TriCcwHullContact(&c->manifold, &c->cache, &dd, NULL, tri, &hull_bvh_local_space, reference_index);
                    ProfZoneEnd;
                    }


                    /* delayed set processed from deepest to shallowest contact. */
                    c->priority = -max_depth*max_depth;
