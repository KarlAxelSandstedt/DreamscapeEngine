...
u32 (*c_contact_methods[C_SHAPE_COUNT][C_SHAPE_COUNT])(struct arena *, struct c_Result *, const struct ds_RigidBodyPipeline *, const struct c_Shape *, const ds_Transform *, const struct c_Shape *, const ds_Transform *, const f32) =
{
...
u32 ds_ShapeContact(struct arena *tmp, struct c_Result *result, const struct ds_RigidBodyPipeline *pipeline, const struct ds_Shape *s1, const struct ds_Shape *s2, const f32 margin)
{
	...
	{
		collision = c_contact_methods[c_s1->type][c_s2->type](tmp, result, pipeline, c_s1, &t1, c_s2, &t2, margin);
	}
	else
	{
		collision = c_contact_methods[c_s2->type][c_s1->type](tmp, result, pipeline, c_s2, &t2, c_s1, &t1, margin);
